...
class ContrastiveModel(Model):
    ...
    def forward(self, batch):
        ...
        if type(self.loss_fn).__name__ == "NCELossTimeAware":
            # Move all needed tensors to device in a single batch operation
            non_blocking = (self.device.type == 'cuda')
            # Prepare time-related data with non-blocking transfers
            subject_id = batch.get("subject_id_int")
            segment_start = batch.get("segment_start_milliseconds")
            
            if subject_id is not None and segment_start is not None:
                # Move to device only if needed
                if subject_id.device != self.device:
                    subject_id = subject_id.to(device=self.device, non_blocking=non_blocking)
                if segment_start.device != self.device:
                    segment_start = segment_start.to(device=self.device, non_blocking=non_blocking)
                
                loss = self.loss_fn(h_1, h_2, subject_id, segment_start)
            else:
                # Fallback if the required time data is missing
