...
class DatasetWrapper(Dataset):
    ...
    def __init__(self, dataset_path, y_dim=None, y_var_type=None, split=None, split_file=None, sub_sample_frac=None,
        ...
        # (8) Make copy of 'x' that is 'x_base' - using numpy array for better performance
        self.data_df['x_base'] = self.data_df['x'].apply(np.array)
        # (9) finally, create list of data_df rows - optimize for speed by creating a list directly
        self.data_list = []
        for _, row in self.data_df.iterrows():
            self.data_list.append({k: row[k] for k in self.data_df.columns})
    ...
    def __getitem__(self, index):
        ...
        if self.data_views is not None:
            # create views - preload the base item for transform to avoid multiple deep copies
            x_base = item['x_base']
            for view_name, view_params in self.data_views.items():
                item[view_name] = self._create_data_view(x_base, view_params.get('apply_transform', False))
        return item
    ...
    def _create_data_view(self, x_base, apply_transform):
        """
        Create alternative "view" of the data
        :param x_base: base data to transform
        :param apply_transform: whether to apply a randomly sampled data transform
        :return: transformed data
        """
        # Avoid deep copy for small performance gain
        x_trf = x_base.copy()
        if apply_transform:
            sample_dict = {'x': x_trf, 'x_base': x_base}
            if 'x_left_buffer' in self.data_list[0] and 'x_right_buffer' in self.data_list[0]:
                sample_dict['x_left_buffer'] = self.data_list[0]['x_left_buffer']
                sample_dict['x_right_buffer'] = self.data_list[0]['x_right_buffer']
            x_trf = self.data_transformer(sample_dict)
        return x_trf
