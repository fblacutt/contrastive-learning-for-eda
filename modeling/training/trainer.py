...
class Trainer:
    ...
    def _log_train_completion(self, train_start_time):
        ...
        # Calculate human-readable training time
        ...
        train_summary = f"TRAINING COMPLETED:\n" \
                        f"- Epochs: {self.train_hist.epoch}\n" \
                        f"- Total steps: {self.train_hist.step}\n" \
                        f"- Training time: {time_str}\n" \
                        f"- Final loss: {self.train_hist.train_loss_epochs[-1]:.6f}\n" \
                        f"- Best loss: {self.train_hist.best_so_far:.6f} (epoch {self.train_hist.best_so_far_epoch})"
