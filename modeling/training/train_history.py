...
class TrainHistory:
    ...
    def increment_epoch(self):
        # Keep tensors on their original device until final computation
        step_losses = self.train_loss_steps[self.epoch]
        if step_losses and isinstance(step_losses[0], torch.Tensor):
            # Convert to float values without unnecessary tensor->numpy conversion
            mean_epoch_loss = torch.mean(torch.stack(step_losses)).item()
        else:
            # Fallback if values are already in list form
            mean_epoch_loss = np.mean(step_losses)
    ...
    def _get_epoch_loss(self):
        ...
        step_losses = self.train_loss_steps[self.epoch]
        if step_losses and isinstance(step_losses[0], torch.Tensor):
            # Calculate mean on device, then get scalar value
