...
class DataTransformer:
    ...
    def __call__(self, sample_dict):
        ...
        """
        # We already have a copy of x from the dataset class
        if self.stochastic_choice:
            # Optimization: For stochastic choice with n_transforms=1, just apply one transform directly
            if self.n_transforms == 1:
                trf_idx = np.random.choice(len(self.trfs))
                return self.trfs[trf_idx](sample_dict).copy()
            
            # For multiple transforms, apply in sequence (less common case)
            for i in range(self.n_transforms):
                trf_idx = np.random.choice(len(self.trfs))
                sample_dict['x'] = self.trfs[trf_idx](sample_dict).copy()
        else:  # apply transforms sequentially
            for i in range(self.n_transforms):
                sample_dict['x'] = self.trfs[i](sample_dict).copy()
